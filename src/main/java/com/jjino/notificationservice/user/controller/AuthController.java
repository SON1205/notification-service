...
public class AuthController {
    ...
    public ResponseEntity<SignupResponse> signup(@Valid @RequestBody SignupRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new SignupResponse(info.userId(), info.username()));
    }

    /**
     * 하이브리드 로그인: Cookie(웹) + body token(앱) 동시 제공. <br> - 웹 클라이언트: HttpOnly Cookie를 자동 전송하여 인증. body token은 무시해야 함. <br>
     * - 앱 클라이언트: body token을 SecureStorage에 저장, Authorization 헤더로 전송. <br>
     * <p>
     * body에 토큰을 포함하더라도 HttpOnly Cookie는 JS에서 접근 불가하므로, <br> 웹에서 localStorage에 저장하지 않는 한 XSS 위험은 제한적.
     */
    @PostMapping("/login")
     ...
     * 쿠키 기반 로그아웃.
     * <p>
     * 브라우저의 HttpOnly Cookie를 삭제한다. <br> 현재는 stateless JWT이므로 서버 사이드 토큰 무효화는 없음. <br> 앱 클라이언트는 로컬 토큰을 직접 삭제해야 함. <br> 서버
     * 사이드 무효화가 필요하면 Refresh Token + Redis 블랙리스트 도입 필요 (Phase 3).
     */
    @PostMapping("/logout")
