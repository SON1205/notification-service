 ...
 * <p>
 * 1. subscribe~send 갭 (알림 유실) <br> - remove() → put() 사이에 send()가 들어오면 emitter가 없어 알림 전송 실패. <br> - 대응: DB에 저장은 완료된
 * 상태이므로, Last-Event-ID 재연결 시 복구됨.
 * <p>
 * 2. send 중 타임아웃 (complete 중복 호출) <br> - get()으로 emitter를 꺼낸 뒤 send() 전에 Tomcat이 타임아웃 처리. - 이미 완료된 emitter에 complete()를
 * 다시 호출하면 IllegalStateException. <br> - 대응: completeQuietly()로 중복 complete 방어.
 * <p>
 * 3. 동시 재구독 경합 (알림 유실) <br> - 같은 userId로 subscribe()가 동시 호출되면 remove~put 사이 윈도우에서 유실 가능. <br> - 대응: 시나리오 1과 동일하게
 * Last-Event-ID로 복구. Phase 3에서 구조적 해소.
 * <p>
