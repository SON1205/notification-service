...
public class NotificationService {
     ...
     * <p>
     * [기존 방식] TransactionTemplate으로 DB 저장만 감싸고 SSE를 직접 호출 → "커밋 후 전송"이라는 의도가 코드 구조에 드러나지 않고, 선언적 @Transactional과 프로그래매틱
     * TransactionTemplate이 혼용되어 일관성 저하
     * <p>
     * [현재 방식] @Transactional + ApplicationEventPublisher + @TransactionalEventListener(AFTER_COMMIT) → DB 커밋이 확정된 후에만
     * SSE 전송이 실행됨이 구조적으로 보장됨 → 서비스는 "저장 + 이벤트 발행"이라는 도메인 책임만 가지고, SSE 전송은 NotificationEventListener에서 처리 (관심사 분리) → 추후
     * WebSocket, FCM 등 전송 채널 추가 시 리스너만 확장하면 됨
     */
    @Transactional
